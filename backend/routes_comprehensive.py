...
async def analyze_question(request: AnalyzerQuestionRequest, db: Session = Depends(get_db)):
    ...
    Enhanced to use the full Quran database via API for more accurate verse matching.
    """
    # Try the enhanced method with full Quran API first
    try:
        analysis_result = await AIAnalyzerService.analyze_with_full_quran(request.question)
    except Exception as e:
        print(f"[Analyzer] Full Quran analysis failed, falling back: {e}")
